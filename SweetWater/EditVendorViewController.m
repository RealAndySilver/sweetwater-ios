...
- (void)viewDidLoad
{
    ...
    nombre=[self.vendorDictionary objectForKey:@"vendorName"];
    email=[self.vendorDictionary objectForKey:@"vendorEmail"];
    password=[self.vendorDictionary objectForKey:@"vendorPassword"];
}
        ...
        if ([indexPath section] == 0) {
            if (indexPath.row<=2) {
                ...
                if ([indexPath row] == 0) {
                    ...
                    playerTextField.text=[self.vendorDictionary objectForKey:@"vendorName"];
                }
                else if ([indexPath row] == 1){
                    playerTextField.placeholder = @"*Requerido*";
                    playerTextField.keyboardType = UIKeyboardTypeAlphabet;
                    playerTextField.returnKeyType = UIReturnKeyDone;
                    playerTextField.text=[self.vendorDictionary objectForKey:@"vendorEmail"];
                }
                if ([indexPath row] == 2) {
                    playerTextField.placeholder = @"*Requerido*";
                    playerTextField.keyboardType = UIKeyboardTypeAlphabet;
                    playerTextField.returnKeyType = UIReturnKeyNext;
                    playerTextField.text=[self.vendorDictionary objectForKey:@"vendorPassword"];
                }
...
-(void)editar{
    ...
    hud.labelText=NSLocalizedString(@"Actualizando Vendedor", nil);
    NSLog(@"El correo es %@",email);
    NSString *params=[NSString stringWithFormat:@"&vendorName=%@&vendorPassword=%@&vendorId=%@&vendorNewEmail=%@",nombre,password,[self.vendorDictionary objectForKey:@"_id"],email];
    ServerCommunicator *server=[[ServerCommunicator alloc]init];
    server.caller=self;
    [server callServerWithPOSTMethod:@"UpdateVendorInformation" andParameter:params httpMethod:@"PUT"];
}
